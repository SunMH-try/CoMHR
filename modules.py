...
class ModelLoader():
    ...
    def save_test_results(self, results, iter, batchsize):
        ...
        for index, (img, pred_vert, gt_vert, f, v) in enumerate(zip(results['imgs'][0], pred_verts, gt_verts, focal, valid)):
            ...
            gt_front = renderer.render_front_view(gt_vert, bg_img_rgb=img.copy())
    ...
    def save_hmr_results(self, results, iter, batchsize):
        ...
        for index, (img, input_img, pred_verts, gt_verts) in enumerate(zip(results['imgs'], results['input_img'], results['pred_verts'], results['gt_verts'])):
            ...
            gt_smpl = renderer.render_front_view(gt_verts[np.newaxis,:,:],
                                                    bg_img_rgb=img.copy())
            # pred_smpl_front = renderer.render_front_view(pred_verts[np.newaxis,:,:], bg_img_rgb=img.copy())
            # pred_smpl_side  = renderer.render_side_view(pred_verts[np.newaxis,:,:])
            # pred_smpl_top   = renderer.render_top_view(pred_verts[np.newaxis,:,:])
